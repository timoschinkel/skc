...
const calculate_skc = (moving_time, elapsed_time) => {
    ...
    const mapping = [
        { hour: 1, stevie_style_ceiling: -1, on_par_ceiling: 0, text: __('Did you really stop on a one hour ride?') },
        { hour: 2, stevie_style_ceiling: -1, on_par_ceiling: 5, text: __('5 minute break max, we all need to stop for a traffic light.') },
        { hour: 3, stevie_style_ceiling: 5, on_par_ceiling: 10, text: __('Three hour ride? A coffee break is allowed, but 20 minutes max!') },
        { hour: 4, stevie_style_ceiling: 6, on_par_ceiling: 8 , text: __('Four hours? A coffee break is allowed, but that\'s it. 20 minutes. Do it.') },
        { hour: 5, stevie_style_ceiling: 6, on_par_ceiling: 8, text: __('With a five hour ride: coffee break and candy bar and fizzy drink is allowed. 25 minutes without movement max.') },
        { hour: 6, stevie_style_ceiling: 6, on_par_ceiling: 8, text: __('Six hour ride? Coffee break and a candy bar and fizzy drink stop is allowed. And a toilet break. 30 minutes. Full throttle.') },
        { hour: 7, stevie_style_ceiling: 7, on_par_ceiling: 10, text: __('Seven hours. Okay, okay. Coffee break and a candy bar and fizzy drink stop allowed. And a toilet break. Or two. 45 minutes.') },
        { hour: 8, stevie_style_ceiling: 8, on_par_ceiling: 11, text: __('Eight hours? Nice job. You\'ll get 50 minutes of "me time". Enjoy.') },
        { hour: 9, stevie_style_ceiling: 8, on_par_ceiling: 11, text: __('Nine hour training? Kudo\'s guaranteed. 60 minutes to be on par with SKC!') },
        { hour: 10, stevie_style_ceiling: 10, on_par_ceiling: 13, text: __('Sick fuck. (Take your time.)') },
    ];

    const skc = Math.round(((elapsed_time - moving_time) / moving_time) * 1000) / 10;
    const hour = Math.round(moving_time / 3600);

    const rank = mapping[Math.min(Math.max(0, hour - 1), mapping.length - 1)];
